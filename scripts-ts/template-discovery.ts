...
// Configuration for a specific template type and language
export interface TemplateConfig {
  directories: string[];
  files: TemplateMetadata[];
}

// Extract metadata from template file comments
export function extractTemplateMetadata(templatePath: string, templateName: string): TemplateMetadata | null {
...
function inferDestinationPath(templateName: string): string | null {
  // Remove .tpl extension and convert to appropriate path
  const baseName = templateName.replace(/\.tpl$/, "");
  
  // Common patterns
  if (baseName.includes("config.go")) return "internal/config/config.go";
  if (baseName.includes("config.py")) return "internal/config/config.py";
  if (baseName.includes("main.go")) return "cmd/main.go";
  if (baseName.includes("main.py")) return "main.py";
  if (baseName.includes("env")) return ".env";
  if (baseName.includes("Dockerfile")) return "Dockerfile";
  if (baseName.includes("docker-compose")) return "docker-compose.yml";
  if (baseName.includes("requirements")) return "requirements.txt";
  if (baseName.includes(".dockerignore")) return ".dockerignore";
  if (baseName.includes("database.py")) return "internal/database/database.py";
  
  // Module patterns - these need special handling
  if (baseName.includes("model.go")) return "internal/{{MODULE_NAME}}/{{MODULE_NAME}}.model.go";
  if (baseName.includes("model.py")) return "internal/{{MODULE_NAME}}/{{MODULE_NAME}}.model.py";
  if (baseName.includes("repository.go")) return "internal/{{MODULE_NAME}}/{{MODULE_NAME}}.repository.go";
  if (baseName.includes("repository.py")) return "internal/{{MODULE_NAME}}/{{MODULE_NAME}}.repository.py";
  if (baseName.includes("service.go")) return "internal/{{MODULE_NAME}}/{{MODULE_NAME}}.service.go";
  if (baseName.includes("service.py")) return "internal/{{MODULE_NAME}}/{{MODULE_NAME}}.service.py";
  if (baseName.includes("use-cases.go")) return "internal/{{MODULE_NAME}}/{{MODULE_NAME}}.use-cases.go";
  if (baseName.includes("use-cases.py")) return "internal/{{MODULE_NAME}}/{{MODULE_NAME}}.use-cases.py";
  
  return null;
}

// Discover templates for a specific language and type
export function discoverTemplates(language: string, type: "service" | "module", templatesDir: string = "."): TemplateConfig {
  ...
  const files: TemplateMetadata[] = [];
  
  // Function to process templates in a directory
  function processTemplateDir(dir: string, isOverride = false) {
...
export function getTemplateConfig(language: string, type: "service" | "module"): TemplateConfig {
