...
import { writeMultipleTemplates, mkdirs, writeTemplatesFromConfig } from "./template-utils.js";
import { getTemplateConfig } from "./template-discovery.js";
export async function createService({ serviceName, cwd = process.cwd(), language = "go", useDiscovery = true, }) {
    ...
    if (useDiscovery) {
        ...
        const { directories, files } = await import(`./language-config/${language}.json`);
        // Create directories
        mkdirs(directories);
    ...
    try {
        ...
        // Try Docker
        try {
            const container = execSync("docker ps --filter ancestor=postgres --format '{{.ID}}' | head -n 1")
                .toString()
                .trim();
            if (!container) {
                // console.error(
                ...
                try {
                    execSync(`docker exec -u postgres ${container} psql -tc \"SELECT 1 FROM pg_database WHERE datname = '${serviceName}'\" | grep -q 1 || docker exec -u postgres ${container} createdb ${serviceName}`);
                }
                catch {
                    return;
                }
