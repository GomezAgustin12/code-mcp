...
func (usecases *UseCases) Register(r *gin.Engine) {
	r.POST("/user", usecases.Create)
	r.GET("/user/:id", usecases.Get)
	r.GET("/user", usecases.List)
	r.PUT("/user/:id", usecases.Update)
	r.DELETE("/user/:id", usecases.Delete)
}
...
func (usecases *UseCases) Update(c *gin.Context) {
	...
	var req CreateUserReq // TODO: Replace with Update req if needed
	if err := c.ShouldBindJSON(&req); err != nil {
		c.JSON(http.StatusBadRequest, ErrorResponse{Error: err.Error()})
		return
	}
